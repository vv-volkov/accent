Ext.define('KitchenSink.view.tree.FilteredTree', {
    ...
    exampleDescription: [
        '<p>This example shows how filters can be applied to the TreeStore of a Tree.</p>',
        '<p>TreeStore filters are added in the same way as other store filters.</p>',
        '<p>The filter function is called on child nodes first so that it can use child node state',
        ' when calculating a parent node\'s filter result.</p>',
        '<p>Folder nodes who\'s child nodes are all filtered out will be hidden.</p>',
        '<p>Regular expressions may be used, eg <code>(tab|tree)panel</code></p>',
        '<p>Use <code><b>useArrows: true</b></code> when filtering so that the last <b>visible</b>',
        ' node does not have an incorrect connector line.</p>'
    ].join(''),
    //</example>
    store: 'Posts',
    rootVisible: false,
