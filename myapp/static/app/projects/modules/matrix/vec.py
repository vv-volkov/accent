...
def equal(u,v):
    ...
    assert u.D == v.D
    for k in u.f:
        if k not in v.f and u.f[k] != 0:
            return False
        elif k in v.f and u.f[k] != v.f[k]:
            return False
    for k in v.f:
        if k not in u.f and v.f[k] != 0:
            return False
        elif k in u.f and v.f[k] != u.f[k]:
            return False
    return True
