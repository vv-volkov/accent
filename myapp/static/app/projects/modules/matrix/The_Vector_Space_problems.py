...
def GF2_span(D, S):
    ...
    import itertools
    import operator  
    return {functools.reduce(operator.add, map(operator.mul,x,S)) for x in itertools.product(range(2), repeat = len(S)) }   
